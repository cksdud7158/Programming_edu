...
select ename, sal, hiredate from emp where lower(ename)='adams';

-- 2. 7년 이상 장기 근속한 직원들의 이름, 입사일, 급여, 근무 년차를 조회하시오.
select ename, hiredate, sal, floor(datediff(curdate(),hiredate)/365) 근무년차 from emp where floor(datediff(curdate(),hiredate)/365)>=7;

-- 3. 각 부서별 인원수를 조회하되 인원수가 5명 이상인 부서만 출력 되도록 하시오
select deptno, count(*)  from emp  group by deptno having count(*)>=5;

/*4. 각 부서별 최대급여와 최소급여를 조회하시오.
단, 최대급여와 최소급여가 같은 부서는 직원이 한명일 가능성이 높기 때문
에 조회결과에서 제외시킨다.
*/
select distinct max(sal), min(sal) from emp group by  DEPTNO;
...
*/
select * from emp where year(HIREDATE)=1981 and DEPTNO in(10,20,30) and sal between 1500 and 3000 and comm is not null order by hiredate;
select * from emp where year(HIREDATE) like '%81' 
