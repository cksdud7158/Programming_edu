...
public class FootBallDAOImpl implements FootballDAO {
	...
	public VoteVO findVoteByTeamId(int teamId) throws SQLException {
		...
		try {
			...
			if (rs.next()) {
				...
				vVo.setmVo(findMatchByVoteId(rs.getInt("teamId")));
				vVo.setVrVoList(findVoteResult(rs.getInt("teamId")));
			}
		} finally {
			closeAll(rs, ps, conn);
		}
	...
	@Override
	public ArrayList<VoteResultVO> findVoteResult(int voteId) throws SQLException {
		Connection conn = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	@Override
	public TeamVO getSelectedTeam(String userId, int teamId) throws SQLException {
		Connection conn = null;
		PreparedStatement ps = null;
		ResultSet rs1 = null;
		...
		try {
			conn = getConnection();
			// userid와 teamid로 팀의 정보(team & teaminfo & teammember) 가져와야함
			// team, teaminfo 조인하여 teamVO 생성
			String query = "select a.*, b.stadiumName, b.stadiumAddr, b.stadiumCost, b.stadiumType from (select t.*, tm.teammemberId, tm.regDate, tm.manager, tm.participation, tm.status, tm.userid, tif.mannerScore, tif.matchNum, tif.ranking, tif.memberNum, tif.teamScore, tif.winningScore from team t, teammember tm, teaminfo tif where t.teamid=tm.teamid and tm.teamid=tif.teamid and tm.userid=? and t.teamid=?) a, stadium b where a.stadiumid=b.stadiumid;";
			ps = conn.prepareStatement(query);
			ps.setString(1, userId);
			ps.setInt(2, teamId);
			rs1 = ps.executeQuery();
			if (rs1.next()) { // 팀아이디, 유저아이디로 조회하면 결과는 유일, 그래서 if
				tVo = new TeamVO(teamId, rs1.getString("teamname"), rs1.getString("emblem"), rs1.getString("area1"),
						rs1.getString("area2"), rs1.getString("area3"), rs1.getInt("stadiumId"));
				// setter로 TeamVO에 TeamInfo 주입
				tVo.setTi(
						new TeamInfoVO(teamId, rs1.getInt("mannerScore"), rs1.getInt("matchNum"), rs1.getInt("ranking"),
								rs1.getInt("memberNum"), rs1.getInt("teamScore"), rs1.getInt("winningScore")));
				// tmvList에 결과 주입 후 setter로 TeamVO에 TeamMemberVO 주입
				tmvList.add(new TeamMemberVO(userId, teamId, rs1.getString("regDate"), rs1.getInt("manager"),
						rs1.getFloat("participation"), rs1.getInt("status")));
				tVo.setTmvList(tmvList);
				
				// setter로 TeamVO에 stadiumVO 주입
				tVo.setsVo(new StadiumVO(rs1.getInt("stadiumId"), rs1.getString("stadiumName"),
						rs1.getString("stadiumAddr"), rs1.getInt("stadiumCost"), rs1.getString("stadiumType")));
			}
		} finally {
			closeAll(rs1, ps, conn);
		}
	...
	public static void main(String[] args) throws SQLException {
		...
		// dao.registerUser(new UserVO("id2", "pass1", "name1", "phoneNum1", "photo1",
		// "ssn1", "nickName1", 'm', "email1", "addr1", "favTeam11", "favTeam21",
		// "korea"), new PlayerInfoVO("id2","st", "left", 180, 70, 100, 100, 100, 100,
		// 100, 100));
		// dao.registerPlayerInfo(new PlayerInfoVO("id2","st", "left", 180, 70, 100,
		// 100, 100, 100, 100, 100));

		// requestToJoin
		// dao.requestToJoin(new PlayerInfoVO("id1", "st","right", 180, 70, 100, 100,
		...
		// insertMatchVO
		// dao.insertMatch(new MatchVO(2, 2, "2020-07-12", 3, "homeSquad", "awaySquad",
		// 7));
		...
		// update
		// UserVO uVo = new UserVO("aaa", "aaa", "010-5621-4472", "img/aaa.jpg", "이재헌",
		// "111@gmail.com", "답십리", "바르셀로나", "레알마드리드", new PlayerInfoVO("LWF", "왼발", 190,
		// 80, 1, 80, 80, 80, 80, 80, 80, 0));
		// dao.updateUser(uVo);
